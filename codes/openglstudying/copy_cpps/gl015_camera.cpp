...
//  * LastEditTime: 2020-02-21 22:59:18
...
// 3 parameters to be set: model, projection, view
int main(){
    ...
    glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);// capture and hide mouse
    ...
    */
    ...
    while (!glfwWindowShouldClose(window)){
        ...
        */
        ...
        view_rotation = glm::lookAt(cameraPos, cameraPos + cameraFront, cameraUp);
        unsigned int viewLoc  = glGetUniformLocation(shaderProgramTexture01.ID, "view");
        glUniformMatrix4fv(viewLoc, 1, GL_FALSE, &view_rotation[0][0]);

        //projection matrix
        glm::mat4 projection  = glm::mat4(1.0f);
        projection = glm::perspective(glm::radians(fov), 1.0f * (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
        // 只改变FOV，越小，显示的区域约小，物体越近，看起来越大。
        //宽高比变大，会变上下长，变小会变左右长
        unsigned int projectionLoc  = glGetUniformLocation(shaderProgramTexture01.ID, "projection");
        glUniformMatrix4fv(projectionLoc, 1, GL_FALSE, &projection[0][0]);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos){
    ...
    yaw += xoffset * sensitivity;//俯仰角(Pitch)、偏航角(Yaw)和滚转角(Roll)
